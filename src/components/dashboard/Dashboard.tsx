...
const Dashboard: React.FC = () => {
  ...
  const fetchBookings = async () => {
    try {
      const { data } = await axios.get<Booking[]>(`${baseUrl}/customer-login/all`);
      setBookings(data);
    } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      if (editingBooking) {
        await axios.put(`${baseUrl}/customer-login/all`, formData);
      } else {
        await axios.post(`${baseUrl}/customer-login/add`, formData);
      }
      fetchBookings();
      handleCloseDialog();
    } catch (err) {
  ...
  const upcomingBookings = bookings.filter(b => new Date(b.date) >= new Date(today)).length;
    ...
    <Box sx={{ backgroundColor: '#f5f5f5', minHeight: '100vh', py: 4 }}>
      <Container maxWidth="lg">
        ...
        <Box mb={4} display="flex" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={2}>
          <Box>
            <Typography variant="h4" fontWeight="bold">Salon Dashboard</Typography>
          ...
          <Button
            ...
            sx={{ borderRadius: '0.75rem', textTransform: 'none', px: 3, py: 1.5,
                 backgroundColor: '#e91e63', '&:hover': { backgroundColor: '#d81b60' } }}
          >
            New Booking
          </Button>
        </Box>
        ...
        <Grid container spacing={3} mb={4}>
          <Grid item xs={12} md={3}><StatsCard title="Today's Bookings" value={todayBookings} icon={<TodayIcon />}   color="#e91e63" /></Grid>
          <Grid item xs={12} md={3}><StatsCard title="Total Bookings"   value={totalBookings} icon={<ContentCutIcon />} color="#2196f3" /></Grid>
          <Grid item xs={12} md={3}><StatsCard title="Confirmed"       value={confirmedBookings} icon={<ScheduleIcon />}  color="#4caf50" /></Grid>
          <Grid item xs={12} md={3}><StatsCard title="Upcoming"        value={upcomingBookings} icon={<CalendarIcon />}  color="#ff9800" /></Grid>
        </Grid>
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ backgroundColor: '#f8f9fa' }}>
                {['Client Name','Date','Time','Status','Actions'].map(h => (
              ...
              {filteredBookings.length === 0 ? (
                ...
                filteredBookings.map((b) => (
                  <TableRow key={b.id} hover>
                    ...
                    <TableCell>
                      <Chip label={b.status} color={getStatusColor(b.status)} size="small" sx={{ borderRadius: '0.5rem' }} />
                    </TableCell>
                    <TableCell align="right">
                      <IconButton size="small" onClick={() => handleOpenDialog(b)} sx={{ color: '#2196f3' }}>
                        <EditIcon />
                      </IconButton>
                      <IconButton size="small" onClick={() => handleDelete(b.id)} sx={{ color: '#f44336' }}>
                        <DeleteIcon />
                      </IconButton>
                    </TableCell>
                  </TableRow>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
          ...
          <DialogContent>
            <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
              <TextField
                label="Client Name" fullWidth required
                value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              />
              <TextField
                type="date" label="Date" fullWidth required
                value={formData.date}
          ...
          <DialogActions sx={{ p: 3 }}>
            ...
            <Button
              ...
              sx={{ textTransform: 'none', backgroundColor: '#e91e63', '&:hover': { backgroundColor: '#d81b60' } }}
            >
